...
mPanel.port.on('api-call',function(data) {
//	console.log('api-call '+data.type+' '+data.api.method);
	function onSuccess(result) {
		if (data.onSuccess)
			mPanel.port.emit(data.onSuccess,{call: data,reply:result});
	}
	function onFailure(result) {
		if (data.onFailure)
			mPanel.port.emit(data.onFailure,{call: data,reply:result});
	}
  ...
  contentScriptOptions: {
	...
	onAttach: function(worker) {
		...
		worker.port.on('nzbget-add', function(data) {
			nzbgAPI.call('append',[data.title+'.nzb', data.url, data.category, 0, false, false, '', 0, 'SCORE'],
				function(result) { // onSuccess
					worker.port.emit('nzbget-added',{success:true,request: data, api:result});
				},
				function(result) { // onFailure
					worker.port.emit('nzbget-added',{success:false,request: data, api:result});
				});
		});
		worker.port.on('sabnzbd-add', function(data) {
			sabAPI.call('addurl',{name: data.url,nzbname: data.title,cat: data.category},
				function(result) { // onSuccess
					worker.port.emit('sabnzbd-added',{success:true,request: data, api:result});
				},
				function(result) { // onFailure
					worker.port.emit('sabnzbd-added',{success:false,request: data, api:result});
				});
		});
	}
});
...
function onPrefChange(prefName) {
	...
	nzbgAPI.url = (sp.prefs.nzbg_ssl?'https':'http')+'://'+sp.prefs.nzbg_ip+':'+sp.prefs.nzbg_port+'/jsonrpc';
	nzbgAPI.username = sp.prefs.nzbg_user;
	nzbgAPI.password = sp.prefs.nzbg_pass;
	nzbgAPI.timeout = sp.prefs.connection_timeout * 1000;
	sabAPI.url = (sp.prefs.sab_ssl?'https':'http')+'://'+sp.prefs.sab_ip+':'+sp.prefs.sab_port+'/sabnzbd/api'
...
//sabAPI.call('version',null,apiSuccess,apiFailure);
//nzbgAPI.call('status',[0],apiSuccess,apiFailure);
//nzbgAPI.call('listgroups',[0],apiSuccess,apiFailure);
//nzbgAPI.call('pausedownload',[],apiSuccess,apiFailure);
