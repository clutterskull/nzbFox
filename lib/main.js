/* global Task, Downloads, CustomizableUI */
...
function log (msg) {
  if (sp.prefs.dev) { console.log(msg); }
}

function parseCustomIndexersPref () {
// parse private newznab list
  let indexers = [];
  if (sp.prefs.custom_indexers !== '') { indexers = sp.prefs.custom_indexers.replace(/ /g, '').split(','); }

  for (let i = 0; i < indexers.length; ++i) {
    if (indexers[i] !== '') { indexers[i] = '*.' + indexers[i]; }
  }
  return indexers;
}
...
function apiCall (sender, data) {
  ...
  function onSuccess (result) {
    if (data.onSuccess) { sender.port.emit(data.onSuccess, {call: data, reply: result}); }
  }
  function onFailure (result) {
    if (data.onFailure) { sender.port.emit(data.onFailure, {call: data, reply: result}); }
  }
...
var mButton = ToggleButton({
  ...
  onChange: function (state) {
    if (state.checked) {
      if (sp.prefs.sab_enabled || sp.prefs.nzbg_enabled) { mPanel.show({position: mButton}); } else {
        showAOMOptions();
  ...
  if (sp.prefs.dl_notification_type === 'desktop' || sp.prefs.dl_notification_type === 'both') {
    notifications.notify({
      ...
      onClick: function (data) {
        if (data === 'sab') { tabs.open(sp.prefs.sab_url); } else
                if (data === 'nzbg') { tabs.open(sp.prefs.nzbg_url); }
      }
    });
  }
}
...
function createPageMod () {
  if (typeof PM !== 'undefined') { PM.destroy(); }
  PM = pageMod.PageMod({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on('api-call', function (data) {
        apiCall(worker, data);
      });
...
let cm = new ChromeMod({ // eslint-disable-line
