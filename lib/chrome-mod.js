...
const { getTabs, getTabContentWindow, getTabForContentWindow, getURI: getTabURI } = require('sdk/tabs/utils');
...
const modOptions = {
  ...
  include: {
    ...
    ok: (rule) => {
      if (isRegExpOrString(rule)) { return true; }
      if (Array.isArray(rule) && rule.length > 0) { return rule.every(isRegExpOrString); }
      return false;
    },
  ...
  exclude: {
    ...
    ok: (rule) => {
      if (isRegExpOrString(rule) || isUndefined(rule)) { return true; }
      if (Array.isArray(rule) && rule.length > 0) { return rule.every(isRegExpOrString); }
      return false;
    },
...
const ChromeMod = Class({
  ...
  setup: function ChromeMod (options) {
    ...
    if (has(model.attachTo, 'existing')) { applyOnExistingDocuments(mod); }
  },

  dispose: function () {
    let style = styleFor(this);
    if (style) { detach(style); }

    for (let i in this.include) { this.include.remove(this.include[i]); }
...
function onContentWindow ({ subject: window }) {
  ...
  if (pagemods.size === 0) { return; }

  // Wait for 'load' event, otherwise 'window.document.URL' is 'about:blank' for popup chrome:// windows (e.g "chrome://mozapps/content/downloads/unknownContentType.xul")
  function onContentWindowLoad (event) {
    window.removeEventListener('load', onContentWindowLoad, false);

    for (let pagemod of pagemods) {
      if (modMatchesURI(pagemod, window.document.URL)) { onContent(pagemod, window); }
    }
  }
  window.addEventListener('load', onContentWindowLoad, false);
}
...
function applyOnExistingDocuments (mod) {
  getTabs().forEach(tab => {
    ...
    if (has(mod.attachTo, 'top') && modMatchesURI(mod, uri)) { onContent(mod, window); }
...
function onContent (mod, window) {
  ...
  domOn(window, eventName, function onReady (e) {
    if (e.target.defaultView !== window) { return; }
