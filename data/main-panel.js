...
function log (msg) {
  if (noSDK) { console.log(msg); } else { self.port.emit('log', msg); }
}
...
//        console.log('s: '+Tab.download.speed+' p: '+Tab.download.percent+' t: '+Tab.download.time+' z: '+Tab.download.paused);
    if (Tab.download.speed > 0 && Tab.queue.time > 0 && !Tab.download.paused) {
      ...
      if (!progressWidgetVisible) { showProgressWidget(); }
      updateProgressWidget(Tab.download.percent, timeToStringS(Tab.queue.time), (Tab.queue.num > 1 ? Tab.queue.num : 'nzb'));
      return true;
    } else { aTabIsPaused = aTabIsPaused || Tab.download.paused; }
  }) === false) {
        // No tabs are downloading
    if (progressWidgetVisible) { hideProgressWidget(); }
...
function updateProgressWidget (percentage, timeleft, counter) {
  if (progressWidgetVisible) { self.port.emit('updateProgressWidget', [percentage, timeleft, counter]); }
}

function onPrefChange ([prefName, prefValue]) {
  self.options.prefs[prefName] = prefValue;

  if (prefName === 'theme') { $('#theme').attr('href', 'theme.' + self.options.prefs.theme + '.css'); } else
    if (prefName === 'sab_enabled' || prefName === 'nzbg_enabled') {
      if (prefValue) {
        if (prefName === 'nzbg_enabled') { new nzbgTab(); } // eslint-disable-line
        if (prefName === 'sab_enabled') { new sabTab(); } // eslint-disable-line
      } else {
...
self.port.on('show', function () {
  ...
  Tabs.forEach(function (Tab) {
    Tab.statusTimerInterval(self.options.prefs.refresh_active);
  });
});
self.port.on('hide', function () {
  ...
  Tabs.forEach(function (Tab) {
    Tab.statusTimerInterval(Tab.download.speed > 0 ? self.options.prefs.refresh_active : self.options.prefs.refresh_idle);
  });
});
...
function Tab (type) {
 /// / Tab Variables
  ...
  this.errorEle = this.content.find('#error');
    // Buttons
  this.btnTogglePauseEle = this.content.find('#togglePause').button().click(function () {
    _this.btnTogglePause_Click();
  });
  this.btnRefreshEle = this.content.find('#refresh')
        .button({icons: {primary: 'ui-icon-refresh'}})
        .click(function () {
          _this.btnRefresh_Click();
        });
  this.btnShowMenuEle = this.content.find('#showMenu')
        .button({text: false, icons: {primary: 'ui-icon-triangle-1-n'}})
        .click(function () {
          _this.btnShowMenu_Click();
        });
  this.btnOpenEle = this.content.find('#open')
    ...
    // Action Menu
  this.menuEle = this.content.find('ul#menu')
        .menu({position: {my: 'left bottom', at: 'right bottom'}})
        .hide();
    // Action Menu Items
  this.menuSpeedLimit = this.content.find('li#speedLimit')
        .click(function () {
          _this.menuSpeedLimit_Click();
        });

  this.menuPauseFor = this.content.find('li#pause-custom')
        .click(function () {
          _this.menuPauseFor_Click();
        });
  this.menuPauseFor5m = this.content.find('li#pause-5m')
        .click(function () {
          _this.pause(5);
        });
  this.menuPauseFor15m = this.content.find('li#pause-15m')
        .click(function () {
          _this.pause(15);
        });
  this.menuPauseFor30m = this.content.find('li#pause-30m')
        .click(function () {
          _this.pause(30);
        });
  this.menuPauseFor60m = this.content.find('li#pause-60m')
        .click(function () {
          _this.pause(60);
        });
...
Tab.prototype.btnTogglePause_Click = function () {
...
Tab.prototype.btnRefresh_Click = function () {
  this.refreshStatus();
};
...
Tab.prototype.startStatusTimer = function () {
  ...
  if (this.statusTimer_interval > 0) {
    this.statusTimer = window.setTimeout(function () {
      _this.refreshStatus(this.statusTimer_interval * 1000);
    });
  }
};
...
Tab.prototype.startHistoryTimer = function () {
  ...
  this.historyTimer = window.setTimeout(function () {
    _this.refreshHistory(this.historyTimer_interval * 1000);
  });
};
...
Tab.prototype.onStatusParsed = function () { // Shared code for post-status parsing
    // Update Tab UI
  var nameStr = this.download.name === ''
    ? ''
    : this.download.name + (
      (this.download.category !== '*' && this.download.category !== '')
        ? ' (' + this.download.category + ')'
        : ''
    );
...
Tab.prototype.error = function (message) {
  this.errorEle.html(message);
};
...
function sabTab (type) {
  ...
  this.menuOnFinishNothing = this.content.find('li#finish-nothing')
        .click(function () { _this.setFinishAction(''); });
  this.menuOnFinishShutdown = this.content.find('li#finish-shutdown')
        .click(function () { _this.setFinishAction('shutdown_program'); });
  this.menuOnFinishShutdownPC = this.content.find('li#finish-shutdownPC')
        .click(function () { _this.setFinishAction('shutdown_pc'); });
  this.menuOnFinishStandbyPC = this.content.find('li#finish-standbyPC')
        .click(function () { _this.setFinishAction('standby_pc'); });
  this.menuOnFinishHibernatePC = this.content.find('li#finish-hibernatePC')
        .click(function () { _this.setFinishAction('hibernate_pc'); });
  this.menuOnFinishScript = this.content.find('li#finish-script')
        .click(function () { _this.menuOnFinishScript_Click(); });
}
...
sabTab.prototype.menuOnFinishScript_Click = function () {
  ...
  for (var i = 0; i < this.lastStatus.queue.scripts.length; ++i) {
    if (this.lastStatus.queue.scripts[i].endsWith('.py') || this.lastStatus.queue.scripts[i].endsWith('.bat') || this.lastStatus.queue.scripts[i].endsWith('.cmd')) {
      options.push(
                $('<option>', {value: 'script_' + this.lastStatus.queue.scripts[i], selected: (this.lastStatus.queue.finishaction === 'script_' + this.lastStatus.queue.scripts[i])}).text(this.lastStatus.queue.scripts[i])
            );
    }
  }
...
sabTab.prototype.parseStatus = function (api) {
  ...
  if (!api.queue.finishaction) {
    ...
    if (api.queue.finishaction === 'shutdown_program') { this.menuOnFinishShutdown.css('color', '#FF8C00'); } else
    if (api.queue.finishaction === 'shutdown_pc') { this.menuOnFinishShutdownPC.css('color', '#FF8C00'); } else
    if (api.queue.finishaction === 'standby_pc') { this.menuOnFinishStandbyPC.css('color', '#FF8C00'); } else
    if (api.queue.finishaction === 'hibernate_pc') { this.menuOnFinishHibernatePC.css('color', '#FF8C00'); } else
    if (api.queue.finishaction.startsWith('script_')) { this.menuOnFinishScript.css('color', '#FF8C00'); }
  ...
  for (var i = 0; i < api.queue.slots.length; ++i) {
    ...
    if (item.status === 'Downloading' || item.status === 'Queued') { this.queue.num++; }
  } // All item loop
};
...
sabTab.prototype.parseHistory = function (api) {
  ...
  if (this.lastHistoryID === '') {
    ...
    if (api.history.slots[0].nzo_id !== this.lastHistoryID && (time() - api.history.slots[0].completed) < (self.options.prefs.refresh_history * 3) /* finished in past _ secs */ && (api.history.slots[0].status === 'Completed' || api.history.slots[0].status === 'Failed')) {
      ...
      if (api.history.slots[0].fail_message !== '') { stats += '<br/><font color="red">' + escapeHtml(api.history.slots[0].fail_message) + '</font>'; }
...
sabTab.prototype.pause = function (mins) {
  if (mins > 0) { api.call(this, 'config', {name: 'set_pause', value: mins}, this.refreshStatus); } else { api.call(this, 'pause', {}, this.refreshStatus); }
};
      ...
      if (this.lastHistoryID === '') {
        ...
        break;
            ...
            if (api.result[i].NZBID !== this.lastHistoryID && (time() - api.result[i].HistoryTime) < (self.options.prefs.refresh_history * 3) /* finished in past _ secs */) {
              ...
              if (api.result[i].UnpackTimeSec > 0) { stats += '<br/>Unpacked in ' + escapeHtml(timeToStringL(api.result[i].UnpackTimeSec)); }

              if (status !== 'SUCCESS') { stats += '<br/><font color="red">' + escapeHtml(nzbgStatusToString(api.result[i].Status)) + '</font>'; }
...
nzbgTab.prototype.pause = function (mins) {
  ...
  } else { api.call(this, 'pausedownload', [], this.refreshStatus); }
};
...
const dialog = (new function () {
  ...
  this.speedLimit.open = function (speedLimit, onClose) {
    ...
    if (onClose) { this.parent().find('button:eq(1)').one('click', onClose); }
  };
  ...
  this.finishScript.open = function (finishScript, onClose) {
    ...
    for (var i = 0; i < finishScript.length; ++i) { scriptSelect.append(finishScript[i]); }
...
$(function () {
  ...
  if (self.options.prefs.nzbg_enabled) { onPrefChange(['nzbg_enabled', true]); }
  if (self.options.prefs.sab_enabled) { onPrefChange(['sab_enabled', true]); }
