...
function CreateButtons (title, category, password, url, wrapHTML) {
  function CreateButton (type, title, category, password, url, wrapHTML) {
    ...
    function onFailure (api) {
      ...
      window.alert('Unable to send NZB to download client' + '\n' + 'Message: ' + api.message + '\n\n' + JSON.stringify(api.query));
    }
  ...
  var result = $();
  if (self.options.prefs.nzbg_enabled) { result = result.add(CreateButton('nzbg', title, category, password, url, wrapHTML)); }
  if (self.options.prefs.sab_enabled) { result = result.add(CreateButton('sab', title, category, password, url, wrapHTML)); }
  ...
  for (let i = 0; i < self.options.indexers.length; ++i) {
    ...
    if (window.location.hostname.endsWith(self.options.indexers[i])) {
      ...
      var eachNewznabDownload = (index) => {
        ...
        var dlkey = dlURL.match(/[a-f0-9]{32}/i);
        if (domain === 'nzbgeek.info') dlkey = getURLParameter('guid', $(thisRow).find('a[href*="guid="]').attr('href'));
        var urlPassword = dlURL.match(/(?:%7B%7B|\{\{)(.*)(?:%7D%7D|\}\})/); // fetch password between {{password}} or %7B%7Bpassword%7D%7D in URL
        if (urlPassword != null) urlPassword = unescape(urlPassword[1]); else urlPassword = ''; // Password is either an array or null
        ...
        let downloadButtons = CreateButtons(Title, Category, urlPassword, URL, wrapHTML);
        if (domain === 'dognzb.cr') {
        ...
        $('td.file').each(function (index) {
          $(this).prepend(CreateButtons($(this).text(), self.options.prefs.cat_anime, '', $(this).find('a').prop('href')));
            ...
            if (domain === 'binsearch.info') {
              ...
              var btnSelector = '';
              if (domain === 'dognzb.cr') btnSelector = 'div.dog-icon-download';
              else if (domain === 'nzbgeek.info') btnSelector = 'a[href*="&api="][title="Download NZB"]';
              else btnSelector = 'div.icon.icon_nzb, a.icon.icon_nzb';

              if (domain === 'dognzb.cr') apiURL = 'api.dognzb.cr';
              else if (domain === 'nzbgeek.info') apiURL = 'api.nzbgeek.info';
              else if (domain === 'nzb.su') apiURL = 'api.nzb.su';
              else if (domain === 'oznzb.com') apiURL = 'api.oznzb.com';
              else if (domain === 'nzbplanet.net') apiURL = 'api.nzbplanet.net';
              else apiURL = window.location.hostname;
